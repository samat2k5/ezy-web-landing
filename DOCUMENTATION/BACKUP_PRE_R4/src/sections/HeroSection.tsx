...
export const HeroSection: React.FC<HeroSectionProps> = ({ onOpenDemo, onOpenTrial }) => {
  ...
  return (
    <section className="relative pt-32 pb-20 lg:pt-40 lg:pb-32 bg-slate-950 text-white overflow-hidden border-b border-slate-900">
      
      {/* Ambient Multi-colour Glow Background Effects */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[500px] bg-gradient-to-tr from-blue-600/15 via-cyan-500/10 to-emerald-500/15 blur-[160px] rounded-full pointer-events-none" />
      <div className="absolute top-1/3 right-10 w-[500px] h-[400px] bg-indigo-600/15 blur-[150px] rounded-full pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Top Tagline Badge */}
        <div className="flex justify-center mb-6">
          <div className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-slate-900/90 border border-slate-800 text-xs text-slate-300 backdrop-blur-md shadow-lg">
            <span className="flex h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="font-bold text-emerald-400 uppercase tracking-wider text-[11px]">ezyHR SaaS Platform</span>
            <span className="text-slate-600">•</span>
            <span className="text-slate-300 font-medium">Built for Singapore Employment Requirements</span>
          </div>
        </div>
        ...
        <div className="text-center max-w-4xl mx-auto space-y-6">
          ...
          {/* Primary & Secondary Conversion CTAs with Polish */}
          <div className="pt-3 flex flex-col sm:flex-row items-center justify-center gap-3.5 sm:gap-4">
            <button
              onClick={onOpenDemo}
              className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-emerald-500 via-teal-500 to-blue-600 hover:from-emerald-400 hover:to-blue-500 text-slate-950 font-extrabold rounded-2xl text-base shadow-xl shadow-emerald-500/25 transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2.5 ring-1 ring-emerald-400/40"
            >
            ...
            <button
              onClick={onOpenTrial}
              className="w-full sm:w-auto px-8 py-4 bg-slate-900 hover:bg-slate-800 text-slate-200 hover:text-white font-bold rounded-2xl text-base border border-slate-800 transition-all flex items-center justify-center gap-2"
            >
              Start Free Trial
            </button>
          </div>
        ...
        {/* Large Product Hero Interface Composition with Floating Micro-Badges */}
        <div className="mt-14 lg:mt-20 max-w-6xl mx-auto relative">
          
          {/* Floating Micro-Badge 1: Top Right */}
          <div className="hidden sm:flex absolute -top-5 -right-4 z-20 items-center gap-2.5 px-4 py-2 rounded-2xl bg-slate-900/95 border border-emerald-500/40 text-xs font-bold text-emerald-400 shadow-2xl backdrop-blur-md animate-float">
            <CheckCircle2 className="w-4 h-4 text-emerald-400" />
            <span>Payroll Batch Approved</span>
          </div>

          {/* Floating Micro-Badge 2: Left Middle */}
          <div className="hidden sm:flex absolute top-1/2 -left-6 z-20 items-center gap-2.5 px-4 py-2 rounded-2xl bg-slate-900/95 border border-cyan-500/40 text-xs font-bold text-cyan-400 shadow-2xl backdrop-blur-md animate-float" style={{ animationDelay: '1.5s' }}>
            <Clock className="w-4 h-4 text-cyan-400" />
            <span>Attendance Recorded</span>
          </div>

          {/* Floating Micro-Badge 3: Bottom Right */}
          <div className="hidden sm:flex absolute -bottom-5 right-10 z-20 items-center gap-2.5 px-4 py-2 rounded-2xl bg-slate-900/95 border border-purple-500/40 text-xs font-bold text-purple-300 shadow-2xl backdrop-blur-md animate-float" style={{ animationDelay: '2.5s' }}>
            <Sparkles className="w-4 h-4 text-purple-400" />
            <span>AI Anomaly Checked</span>
          </div>

          <div className="relative rounded-3xl bg-slate-900 border border-slate-800 shadow-hero-shadow overflow-hidden group">
            ...
            <div className="bg-slate-950 px-5 py-3.5 border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                {/* Official Logo Brand Indicator in Application Chrome */}
