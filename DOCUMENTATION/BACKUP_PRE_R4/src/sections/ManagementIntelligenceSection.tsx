  ...
  return (
    <section className="py-24 bg-white text-slate-900 relative border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Analytics Container */}
        <div className="bg-slate-50 p-7 sm:p-10 rounded-3xl border border-slate-200 shadow-xl space-y-6">
          
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
            <div>
              <div className="flex items-center gap-2.5">
                <h3 className="text-xl font-bold text-slate-900">Management Intelligence Center</h3>
                <span className="px-3 py-0.5 rounded bg-emerald-100 text-emerald-800 text-[10px] font-extrabold border border-emerald-200">
                  DEMO DATA
                </span>
              </div>
            ...
            <div className="flex bg-white p-1.5 rounded-2xl border border-slate-200 text-xs shadow-xs">
              <button
                onClick={() => setSelectedMetric('headcount')}
                className={`px-3.5 py-2 rounded-xl font-semibold transition-all ${
                  selectedMetric === 'headcount' ? 'bg-slate-900 text-white font-bold shadow-xs' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
              ...
              <button
                ...
                className={`px-3.5 py-2 rounded-xl font-semibold transition-all ${
                  selectedMetric === 'payroll' ? 'bg-slate-900 text-white font-bold shadow-xs' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                Payroll Breakdown
              </button>
              <button
                ...
                className={`px-3.5 py-2 rounded-xl font-semibold transition-all ${
                  selectedMetric === 'turnover' ? 'bg-slate-900 text-white font-bold shadow-xs' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                Retention & Stability
              </button>
                ...
                {[
                  ...
                  <div key={idx} className="space-y-1 text-xs sm:text-sm">
                    <div className="flex justify-between text-slate-700 font-medium">
                      <span>{item.dept}</span>
                      <span className="font-mono font-bold text-slate-900">{item.count} staff ({item.pct})</span>
                    </div>
            ...
            <div className="space-y-4 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-5 bg-white rounded-2xl border border-slate-200">
                  ...
                  <span className="text-2xl font-extrabold font-mono text-slate-900 mt-1 block">$198,800.00</span>
