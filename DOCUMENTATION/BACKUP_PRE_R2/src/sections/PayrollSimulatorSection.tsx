...
import { Calculator, DollarSign, FileText, CheckCircle2, Shield, Sparkles } from 'lucide-react';
...
export const PayrollSimulatorSection: React.FC<PayrollSimulatorSectionProps> = ({
  ...
  // Calculate CPF preview based on simplified SG rules for demonstration
  const calculateCpf = () => {
    if (residency === 'EP') {
      return { empCpf: 0, emprCpf: 0, sdl: Math.min(salaryInput * 0.0025, 11.25), netPay: salaryInput };
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold uppercase tracking-wider">
            <Calculator className="w-3.5 h-3.5" /> Interactive Calculator Preview
          </div>
          <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white">
            Test Singapore Payroll Calculations Live
          </h2>
          <p className="text-base sm:text-lg text-slate-300">
            Adjust employee parameters below to see ezyHR's automated CPF and net pay calculations.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center max-w-5xl mx-auto">
          ...
          {/* Controls Panel */}
          <div className="lg:col-span-6 bg-slate-950 p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <h3 className="text-base font-bold text-white">Salary & Statutory Inputs</h3>
              <span className="px-2.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400 text-[10px] font-bold tracking-wider">
                DEMO DATA
              </span>
            </div>
          ...
          {/* Results Summary Box */}
          <div className="lg:col-span-6 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-emerald-400" /> Simulated Calculation Results
              </h3>
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/30 flex justify-between items-center">
                <div>
                  ...
                  <span className="text-[10px] text-slate-400">Disbursed to Employee Bank</span>
                </div>
            ...
            <div className="pt-2">
              ...
              >
                <FileText className="w-4 h-4 text-emerald-400" /> View Sample MOM Itemized Payslip
              </button>
            </div>
          </div>
