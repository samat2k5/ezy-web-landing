...
import { ShieldCheck, MessageSquare, AlertTriangle, FileSearch, TrendingUp, Bell, Bot, CheckCircle2, ArrowRight } from 'lucide-react';
...
export const AiSection: React.FC = () => {
  ...
  return (
    <section id="ai" className="py-24 bg-slate-900 text-white relative border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold uppercase tracking-wider">
            <ShieldCheck className="w-4 h-4" /> Management Intelligence
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {aiCapabilities.map((ai, idx) => {
            ...
            return (
              <div 
                key={idx}
                className="bg-slate-950 p-6 rounded-2xl border border-slate-800 hover:border-emerald-500/40 transition-all flex flex-col justify-between"
              >
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">{ai.desc}</p>
                </div>
              </div>
            );
          })}
