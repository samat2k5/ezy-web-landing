...
export const PricingSection: React.FC<PricingSectionProps> = ({ onOpenDemo, onOpenTrial }) => {
  ...
  const plans = [
    {
      ...
      ctaText: 'Book a Demo for Starter',
    },
    {
      id: 'professional',
      name: 'Professional',
      ...
      ctaText: 'Book a Demo for Professional',
    },
    {
      id: 'enterprise',
      name: 'Enterprise',
  ...
  return (
    <section id="pricing" className="py-24 bg-ambient-ice text-slate-900 relative border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {plans.map((plan) => (
            <div 
              key={plan.id}
              className={`bg-white p-8 sm:p-10 rounded-2xl border relative flex flex-col justify-between transition-all ${
                plan.popular 
                  ? 'border-2 border-emerald-500 shadow-xl shadow-slate-900/10' 
                  : 'border-slate-200 hover:border-slate-300 shadow-sm'
              }`}
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-2xl font-extrabold text-slate-950">{plan.name}</h3>
              ...
              <div>
                <button
                  onClick={onOpenDemo}
                  className={`w-full py-4 px-4 font-extrabold text-sm rounded-xl transition-all shadow-md flex items-center justify-center gap-2 ${
                    plan.popular
                      ? 'bg-gradient-to-r from-emerald-500 to-blue-600 hover:from-emerald-600 hover:to-blue-700 text-slate-950 shadow-emerald-500/20'
        ...
        <div className="mt-14 text-center text-xs text-slate-600 font-semibold flex flex-wrap justify-center items-center gap-8">
